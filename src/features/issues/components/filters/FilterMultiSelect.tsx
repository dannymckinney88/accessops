          ...
          <div className="max-h-60 overflow-y-auto px-1 py-1">
            {groups && groups.length > 0
              ? groups.map((group, groupIndex) => {
                  const groupHeadingId = `${id}-group-${groupIndex}`;
                  return (
                  <div
                    key={group.groupLabel}
                    role={groups.length > 1 ? "group" : undefined}
                    aria-labelledby={groups.length > 1 ? groupHeadingId : undefined}
                  >
                    {groups.length > 1 && (
                      <p
                        id={groupHeadingId}
                        className="px-2 pb-1 pt-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground first:pt-1"
                      >
                        {group.groupLabel}
                      </p>
                    )}
